...
- `SECURITY.md` — accepted-gaps posture on host config execution; KTD3's cwd isolation is the plugin-side answer that doesn't depend on CLI trust flags.
...
- KTD3. **Trust via cwd isolation, not CLI flags.** The vendored CLI is the published package (0.1.3) — `loadConfig` executes `argus-reviewer.config.ts` unconditionally beside env secrets, and `ARGUS_UNTRUSTED` doesn't exist there. So `argus_review` never runs with cwd = a hostile checkout: no `checkout` arg → a plugin-managed empty scratch dir (the PR diff comes from the GitHub API; the index lane just degrades to `inconclusive` evidence). `checkout` passed → `git -C <checkout> archive HEAD | tar -x -C <scratch>` then delete executable configs (`argus-reviewer.config.ts`, `vision-e2e.config.ts`) — the index lane sees real tracked source, committed `.json` configs still load as data, and no `.ts` can execute. `git archive` reads the object DB without running hooks. `trust_checkout: true` → run directly in the real checkout (full fidelity: uncommitted work, intentional `.ts` config) — for repos you'd hand a terminal to; it also still gates `argus_flow` (which executes checkout test files regardless). `ARGUS_UNTRUSTED=1` is still injected — a no-op on 0.1.3, honored if a future release ships trust machinery. Governs R10.
- KTD4. **CLI resolution is part of the trust boundary.** Executing `<checkout>/node_modules/.bin/argus-reviewer` or `npx` with cwd=checkout runs checkout-controlled code — a forged shim runs beside the overlaid secrets regardless of any env flag. So: `hooks.py` `install()` vendors the CLI once (`npm install --prefix <plugin-dir> argus-reviewer-e2e[@<argus_version_pin>]`), and the default untrusted path spawns only `<plugin-dir>/node_modules/.bin/argus-reviewer`. Checkout-local `.bin` and `npx --no-install` are consulted **only** when `trust_checkout` is set; the vendored binary remains the fallback there too. Never bare `npx argus-reviewer` (package `argus-reviewer-e2e` ≠ bin `argus-reviewer`); vendoring at install also absorbs the `npx -p` cold-start download. Governs R6.
- KTD5. **Result shape is a compact narration block, fields-if-present.** The vendored schema is 0.1.3's: `verdict`, `summary`, findings (`file`, `line`, `severity`, `message`, `evidence`), `probes`, `probeLaneSkipped`, `calls`, `visionCostUsd`, `tokens`, `model`, `budgetExceeded`, `skipped`. Narrate `verdict`, `summary`, counts by severity, top-N findings (bug/risk first, N≈10), `model`, `tokens`, `visionCostUsd`, `budgetExceeded`, `skipped`+reason, `probeLaneSkipped` when set, `reportPath`, `posted`/`commentUrl` on post. Newer-schema fields (`category`, `p`, `triage`, `secretsScan`, `findingAdjudication`) are narrated **if present** — forward-compatible with a Jev release, absent on 0.1.3. Raw stdout/stderr tail (≤4KB) only on failure. Governs R4, R5, R8.
- KTD6. **Single flexible `pr` arg; `checkout` means "trusted source for index depth."** Accept `https://github.com/o/r/pull/N`, `o/r#N`, `o/r/pull/N`, or bare `N` (repo derived from the checkout's `origin` remote — bare `N` therefore needs a real checkout; other forms work with no checkout at all). Reject non-`github.com` hosts with "GitHub Enterprise not supported in v0.1". On `argus_review`, `checkout`/`default_checkout` is **optional** and feeds the KTD3 archive-copy (index evidence), never runs as cwd untrusted. On `argus_flow` it's the real checkout the flow executes against. Governs R4, R7.
...
| Checkout `config.ts` executes inside the A0 host beside `OPENROUTER_API_KEY`/`GITHUB_TOKEN` (0.1.3 has no trust flag) | Review never runs cwd=hostile checkout — scratch dir default, `git archive` copy minus executable configs for index depth, direct cwd only under `trust_checkout` (KTD3); README trust warning (R10) |
...
- `prepare_review_cwd(checkout, trusted)` → KTD3: no checkout → fresh `tempfile.mkdtemp` scratch dir; trusted → the real checkout path; untrusted checkout → `git -C <checkout> archive HEAD | tar -x -C <scratch>` then delete `argus-reviewer.config.ts`/`vision-e2e.config.ts` (keeps `.json` configs — they're data). Non-git `checkout` on untrusted → readable error.
- `fresh_report_dir()` → `tempfile.mkdtemp` under the scratch dir or system temp; kept after run (report path is narrated for drill-down).
...
- Fixture provenance: `code-review.json`/`run.json` fixtures are key-asserted against the **vendored version's** report interfaces (0.1.3 — no `--fixture` exists there to capture from) plus a version comment; a `category`/`p`/`triage` superset fixture exercises the if-present narration path (KTD5).
- `prepare_review_cwd` untrusted: fixture checkout with a hostile `argus-reviewer.config.ts` → archive copy lands without the `.ts`, `.json` config survives, `code-review` never executes checkout code. No-checkout → empty scratch dir. Non-git dir → readable error.
...
**Approach:** `class ArgusReview(Tool)` with `async def execute(**kwargs) -> Response`; reads `checkout`/`pr`/`post` from `self.args` (`checkout` optional — falls back to `default_checkout`, else none). Sequences: normalize → preflight (KTD9; AE4 fail-fast) → `prepare_review_cwd` (KTD3 — scratch/archive/real per trust) → resolve CLI → `run_argus(['code-review','--report-dir',tmp], cwd=prepared)` with progress streaming → parse → narration `Response(message=..., break_loop=False)`. On `post:true` success, invoke U4's post lane and append `posted: <commentUrl>` to the message. Prompt fragment declares tool name `argus_review`, JSON args with defaults, narrate-default semantics, "posts a public comment — only pass post:true when the user asked", and "no checkout needed — pass one only to deepen evidence linking".
