...
function windowContent(content: string, line: number | undefined): string {
  if (content.length <= FILE_CAP_BYTES) return content
  if (line === undefined) return `${content.slice(0, FILE_CAP_BYTES)}\n…[truncated]`
...
async function authorProbe(
  ...
  indexPaths: Set<string> | undefined,
): Promise<
  | { probe: AuthoredProbe; costUsd: number; tokens: number }
  | { probe: undefined; reason: string; costUsd: number; tokens: number }
> {
  ...
  const [rawFile, rawExemplar] = await Promise.all([
    fileTrusted
      ? readFile(join(o.cwd, target.file as string), 'utf8').catch(() => undefined)
  ...
  const exemplar =
    exemplarPath === undefined || rawExemplar === undefined
      ? undefined
      : {
          path: exemplarPath,
          content:
            rawExemplar.length > EXEMPLAR_CAP_BYTES
...
function outcomeOf(harness: Harness, r: SandboxRunResult): ProbeOutcome | 'error' {
...
export async function runProbeLane(
  findings: LinkedFinding[],
  o: ProbeLaneOptions,
): Promise<ProbeLaneResult | undefined> {
  const log = o.log ?? (() => undefined)
  if (!o.sandbox.enabled) return undefined
  const skip = (reason: string): ProbeLaneResult => {
  ...
  const basePromise = (
    o.meta?.baseSha === undefined
      ? Promise.resolve(undefined)
      : addBaseWorktree(exec, o.cwd, wtDir, o.meta.baseSha, o.token)
  ).catch(() => undefined)
  ...
  try {
    for (let i = 0; i < targets.length; i++) {
      const target = targets[i] as LinkedFinding
      ...
      if (authored.probe === undefined) {
        records.push(
          record(target, undefined, 'error', `authoring failed: ${authored.reason}`, {
      ...
      try {
        ...
        if (wroteHead) await rm(join(o.cwd, relProbe), { force: true }).catch(() => undefined)
    ...
    if (bd !== undefined) await removeBaseWorktree(exec, o.cwd, wtDir)
  }
  return { records }
}
