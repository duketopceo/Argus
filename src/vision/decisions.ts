    ...
    if (q.type === 'choice') {
      ...
      if (n === 0 || n > MAX_CHOICE_OPTIONS) {
        throw new DecisionError(
          'validation',
          `decide: choice question "${id}" needs 1-${MAX_CHOICE_OPTIONS} options, got ${n}`,
          false,
        )
      }
      ...
      if (n < MIN_SCORE_LEVELS || n > MAX_SCORE_LEVELS) {
        throw new DecisionError(
          'validation',
          `decide: score question "${id}" needs ${MIN_SCORE_LEVELS}-${MAX_SCORE_LEVELS} levels, got ${n}`,
          false,
        )
      }
    } else if (q.type !== 'noul') {
...
function retryDelayMs(res: Response): number {
  ...
  if (retryAfter !== null) {
    ...
    if (Number.isFinite(parsed) && parsed >= 0) return Math.min(parsed * 1000, 30_000)
  }
  return 1_000
}
...
export class DecisionClient {
  ...
  private _onCall: DecisionClientOptions['onCall']
  ...
  async decide(opts: {
    ...
    state: unknown
    questions: Record<string, DecisionQuestion>
  }): Promise<{ answers: Record<string, DecisionAnswer>; cost: CallCost; model: string }> {
    ...
    if (this._trace) body.trace = this._trace

    let lastErr: DecisionError | undefined
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        ...
        debug(
          'decisions',
          `model=${cost.model} provider=${cost.provider} tokens=${cost.tokens} costUsd=${cost.costUsd.toFixed(6)}`,
        )
        this._onCall?.({
          id: data.id ?? '',
          model: cost.model,
