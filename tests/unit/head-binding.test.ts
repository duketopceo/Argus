...
import { classifyHeadBinding, readCheckoutSha } from '../../src/report/manifest.js'
...
describe('head binding', () => {
  ...
  it('degrades to unknown when git identity cannot be read', async () => {
    const sha = await readCheckoutSha('/repo', async () => ({ code: 1, stdout: '', stderr: 'no git' }))
  ...
  it('marks a live review inconclusive when the checkout is not the PR head', async () => {
    ...
    const intendedSha = 'different-head'
    ...
    vi.stubGlobal('fetch', async (input: RequestInfo | URL) => {
      ...
      if (url.includes('/pulls/1/files')) {
        return response([{ filename: 'src.ts', patch: '@@ -1 +1 @@\n-export const value = 1\n+export const value = 2' }])
      }
    ...
    const client: VisionClient = {
      complete: async (_opts: {
        ...
        cost: { model: 'test/model', provider: 'stub', tokens: 1, costUsd: 0, kind: 'code' } as CallCost,
        model: 'test/model',
      }),
    }
    try {
      const code = await main(['code-review', '--report-dir', reportDir], {
        ...
        env: {
          ...
          ARGUS_REVIEWER_TRACE: JSON.stringify({ repo: 'o/r', pr: '1', commit: intendedSha }),
        },
        out: () => undefined,
        err: () => undefined,
        createClient: () => client,
