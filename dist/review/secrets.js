...
async function ensureBaseObject(exec, cwd, baseSha, token) {
...
export async function scanSecrets(opts) {
    ...
    const pLiveByIdx = new Array(candidates.length);
    ...
    if (opts.client !== undefined && candidates.length > 0) {
        try {
            ...
            candidates.forEach((c, i) => {
                questions[`cand_${i}`] = {
                    type: 'noul',
                    instructions: `state[${i}]: is 'literal' a real, usable credential committed to source? ` +
